...
export class RegisterComponent {
  ...
  const file = event.target.files[0];
  if (!file) {
    this.studentImage = null; // مسح أي صورة محفوظة إذا تم إلغاء التحديد
    return;
  }
 ...
 onSubmit() {
  ...
  // إضافة صورة الطالب إلى formData
  if (this.studentImage) {
    formData.append('studentImage', this.studentImage, this.studentImage.name);
  }
